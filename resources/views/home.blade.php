<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>I-Travel home</title>
    <link rel="stylesheet" href="/itravel/css-slide/bootstrap.min.css">
    <link rel="stylesheet" href="/itravel/css-slide/style.css">
    <link rel="stylesheet" href="/itravel/css/cssku.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
</head>
<body>
        <!-- navbar -->
        <nav class="navbar navbar-expand-lg navbar-dark sticky-top p-3" style="background-color: #252C6A;">
            <div class="container">
                <a href="/home"><img class="me-3" src="/itravel/assets/logo unair fix trans.png" alt="Logo Unair" width="50px"></a>
                <a class="navbar-brand" href="/home" style="color: #FFEA7A;"><h3>I-Travel</h3></a>
                <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
                </button>
                <div class="collapse navbar-collapse righting" id="navbarNavDropdown">
                    <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" style="color: #FFEA7A;" href="/home">Beranda</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" style="color: #FFEA7A;" href="/rentform">Peminjaman</a>
                    </li>
                    <li class="nav-item me-2">
                        <a class="nav-link" style="color: #FFEA7A;" href="#footer">Notifikasi</a>
                    </li>
                    <li>
                        <div class="hidden fixed top-0 right-0 sm:block">
                                <li class="nav-item dropdown d-flex">
                                    <img class="centering mt-1 me-1" src="data:image/png;base64," alt="" style="border-radius: 50%; height:30px;">
                                  <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>User 123
                                </a>
                                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                                    <a class="dropdown-item" href="">Profil</a>
                                    <a class="dropdown-item" href="">Riwayat Peminjaman</a>
                                    <a class="dropdown-item" href="/">Keluar</a>
                                    <form id="logout-form" action="" method="POST" class="d-none">
                                    </form>
                                </li>
                        </div>
                    </li>
                    </ul>
                </div>
            </div>
          </nav>

          <div class="contaner-fluid" style="background-image: linear-gradient(rgba(0, 0, 0, 0.5),
          rgba(0, 0, 0, 0.5)), url('https://4.bp.blogspot.com/-3DggWS3Joic/WbvcnRW5ORI/AAAAAAAADlM/_6U3Z3lqd8E1kKkZffN_vlge8oEpVmZFQCEwYBhgL/s1600/Bis%2BUnir.jpg'); height: 80vh; background-size: cover; background-position: center;">
            <div class="banner-content row">
                <div class="col px-5">
                    <h1 class="mt-3" style="color: #fff; text-align: center;">Sistem Informasi Peminjaman Bus Universitas Airlangga<h1>
                </div>
                <div class="col px-4 centering">
                    <div class="card" style="border-radius: 25px; width: 80%; box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.25);">
                        <div class="card-body" style="color: black;">
                            <form class="p-3">
                                <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Pilih Kategori Bus</label>
                                <select class="form-select" aria-label="Default select example">
                                    <option value="1">One</option>
                                    <option value="2">Two</option>
                                    <option value="3">Three</option>
                                  </select>
                                </div>
                                <div class="mb-3">
                                <label for="exampleInputPassword1" class="form-label">Pilih Tanggal Peminjaman</label>
                                <input name="app_date" type="date" class="form-control">
                                </div>
                                <div class="d-grid gap-2 col-6 mx-auto">
                                    <a href="/home" type="submit" class="btn" style="color:#fff; background-color: #252C6A; border-radius:5px;">Cari</a>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
          </div>

          <div class="container my-5">
            <div class="row">

                <div class="col centering p-3">
                    <div class="card" style="width: 18rem; box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.1);">
                        <img src="https://4.bp.blogspot.com/-3DggWS3Joic/WbvcnRW5ORI/AAAAAAAADlM/_6U3Z3lqd8E1kKkZffN_vlge8oEpVmZFQCEwYBhgL/s1600/Bis%2BUnir.jpg" class="card-img-top" alt="...">
                        <div class="card-body">
                          <h5 class="card-title">Bus Flash Unair A</h5>
                          <p class="card-text">Kapasitas : 60 Penumpang</p>
                          <a href="/rentform" class="btn" style="background-color: #252C6A; color: #fff;">Pinjam</a>
                        </div>
                      </div>
                </div>

                <div class="col centering p-3">
                    <div class="card" style="width: 18rem; box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.1);">
                        <img src="https://4.bp.blogspot.com/-3DggWS3Joic/WbvcnRW5ORI/AAAAAAAADlM/_6U3Z3lqd8E1kKkZffN_vlge8oEpVmZFQCEwYBhgL/s1600/Bis%2BUnir.jpg" class="card-img-top" alt="...">
                        <div class="card-body">
                          <h5 class="card-title">Bus Flash Unair B</h5>
                          <p class="card-text">Kapasitas : 30 Penumpang</p>
                          <a href="/rentform" class="btn" style="background-color: #252C6A; color: #fff;">Pinjam</a>
                        </div>
                      </div>
                </div>

                <div class="col centering p-3">
                    <div class="card" style="width: 18rem; box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.1);">
                        <img src="https://4.bp.blogspot.com/-3DggWS3Joic/WbvcnRW5ORI/AAAAAAAADlM/_6U3Z3lqd8E1kKkZffN_vlge8oEpVmZFQCEwYBhgL/s1600/Bis%2BUnir.jpg" class="card-img-top" alt="...">
                        <div class="card-body">
                          <h5 class="card-title">Bus Flash Unair C</h5>
                          <p class="card-text">Kapasitas : 45 Penumpang</p>
                          <a href="/rentform" class="btn" style="background-color: #252C6A; color: #fff;">Pinjam</a>
                        </div>
                      </div>
                </div>

                <div class="col centering p-3">
                    <div class="card" style="width: 18rem; box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.1);">
                        <img src="https://4.bp.blogspot.com/-3DggWS3Joic/WbvcnRW5ORI/AAAAAAAADlM/_6U3Z3lqd8E1kKkZffN_vlge8oEpVmZFQCEwYBhgL/s1600/Bis%2BUnir.jpg" class="card-img-top" alt="...">
                        <div class="card-body">
                          <h5 class="card-title">Bus Flash Unair D</h5>
                          <p class="card-text">Kapasitas : 50 Penumpang</p>
                          <a href="/rentform" class="btn" style="background-color: #252C6A; color: #fff;">Pinjam</a>
                        </div>
                      </div>
                </div>

            </div>
          </div>

        <!-- footer -->
        <footer id="footer" class="container-fluid px-5 pt-5" style="color:#fff;background-color: #252C6A;">
                <!--Footer Top Start-->
        <div class="footer-top section bg-heading pt-70 pb-70">
            <div class="container">
                <div class="row mbn-35">

                    <div class="col-lg-4 col-md-12 col-12 mb-35" style="display:flex;align-items: center; justify-content:center;">
                        <!--Footer Widget Start-->
                        <div class="uptimo-footer-widget">
                            <!--Footer Widget Inner Start-->
                            <div class="footer-widget-inner">

                                <!--Widget About-->
                                <div class="uptimo-footer-widget-about text-center pt-50 pt-sm-0 pt-xs-0">
                                    <a class="footer-logo"><img src="/itravel/assets/logo unair fix trans.png" alt="Logo Unair" style="width: 150px;"></a>
                                    <div class="footer-social mt-2 p-3">
                                        <a class="p-2" style="color: #fff;"><i class="fa fa-facebook"></i></a>
                                        <a class="p-2" style="color: #fff;"><i class="fa fa-twitter"></i></a>
                                        <a class="p-2" style="color: #fff;"><i class="fa fa-linkedin"></i></a>
                                    </div>
                                </div>

                            </div><!--Footer Widget Inner End-->
                        </div><!--Footer Widget End-->
                    </div>

                    <div class="offset-lg-4 col-lg-4 col-md-6 col-12 mb-35 p-3">
                        <!--Footer Widget Start-->
                        <div class="uptimo-footer-widget">
                            <!--Footer Widget Inner Start-->
                            <div class="footer-widget-inner">

                                <!--Widget Title-->
                                <h3 class="widget-title text-white mb-10">Contact Info</h3>
                                <!--Widget Contact-->
                                <div class="uptimo-footer-widget-contact">
                                    <div class="contact-item phone p-2"><i class="fa fa-phone"></i><a href="tel:<% contactInfo.phone %>"></a></div>
                                    <div class="contact-item email p-2"><i class="fa fa-envelope"></i><a href="mailto:<% contactInfo.email %>"></a></div>
                                    <div class="contact-item address p-2"><i class="fa fa-map-marker"></i><p></p></div>
                                </div>

                            </div><!--Footer Widget Inner End-->
                        </div><!--Footer Widget End-->
                    </div>


                </div>
            </div>
        </div>
        <!--Footer Top End-->

        <!--Footer Bottom Start-->
        <div class="footer-bottom section bg-black pt-10 pb-10 mt-3" >
            <div class="container">
                <div class="row">
                    <div class="uptimo-footer-copyright text-center col">
                        <p>Copyright 2022 By Kelompok Bus Uner</p>
                    </div>
                </div>
            </div>
        </div>
        <!--Footer Bottom End-->
        </footer>

          <script src="/itravel/js/bootstrap.min.js"></script>
          <script src="/itravel/js-slide/bootstrap.bundle.min.js"></script>
</body>
</html>
